# A modified version of umap_.py --> Author: Leland McInnes <leland.mcinnes@gmail.com>
#
# Allows extraction of UMAP graph made of fuzzy simplicial sets
...
@numba.jit()
def fuzzy_simplicial_set_with_knn(
    X,
    n_neighbors,
    random_state,
    metric,
    ...
    # Added
    if knn:
        # Check to make sure I'm actually getting the k nearest neighbors (not?) including self
        # Pause umap time
        # Adapted from https://github.com/scikit-learn/scikit-learn/blob/67cbaefcffe8b094f1996336f3bfcc565ccde514/sklearn/neighbors/base.py
        vals = np.ones(n_samples * n_neighbors)
        indptr = np.arange(0, n_samples * n_neighbors + 1, n_neighbors)
        # check to make sure knn_indices is the right shape
        knn_graph = scipy.sparse.csr_matrix((vals, knn_indices.ravel(), indptr),
                                          shape=(n_samples, n_samples))
        knn_time = time.time() - knn_start + knn_time
    
    pause_time = time.time() - pause_start
    # Create fuzzy simplicial graph
    sigmas, rhos = smooth_knn_dist(
        knn_dists, n_neighbors, local_connectivity=local_connectivity
    )
    ...
    result.eliminate_zeros()
#     test = result.todense().astype(np.float64)
#     fig = plt.figure(figsize=(15,15))
#     plt.imshow(test)
    if knn:
        return result, knn_graph, knn_time, pause_time
    else:
        return result, None, None, pause_time
...
class UMAP(BaseEstimator):
    ...
    def get_graphs(self, X, y=None, knn=False):
        """Fit X into an embedded space.
        Optionally use y for supervised dimension reduction.
        Parameters
        ----------
        ...
        if X.shape[0] < 4096:
            ...
            knn_start = time.time()
            dmat = pairwise_distances(X, metric=self.metric, **self._metric_kwds)
            self.graph_, knn_graph, knn_time, pause_time = fuzzy_simplicial_set_with_knn(
                dmat,
                self._n_neighbors,
                random_state,
                "precomputed",
        ...
        umap_time = time.time() - umap_start - pause_time
        return self.graph_, knn_graph, umap_time, knn_time
